...
class CalculationMixin:
    ...
    async def apply_aggregates_to_mapping_data(self, mapping, continuous_updates=False):
        ...
        await self._aggregate_sensor_data(data_by_sensor, mapping, resultdata, audit)
    ...
    async def _aggregate_sensor_data(
        self, data_by_sensor, mapping, resultdata, audit=None
    ):
    ...
    async def async_calculate_zone(
        ...
        # the record shows the values the zone is actually updated with.
        await self._async_write_calc_record(calc_data)
    ...
    async def _async_write_calc_record(self, calc_data) -> None:
        ...
        if record is None:
            return
        if calc_data:
            # After seasonal adjustments: what the zone is actually updated
            # with, in the user's unit system. Only the keys the calculation
            # produced -- a multiplier here means a seasonal adjustment changed
            # it, which is exactly what one looks for in a surprising day.
